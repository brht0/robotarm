...
sun_bore_r=5.0/2 * (1.0 / 0.9825); // Nema 23 D shaft dia
...
ring_teeth=48; // needs to be divisable by both <num_planets> and 2.
...
if(0){
    ...
    difference(){
    union(){
        translate([0, 0, -3.0]) cylinder(r=housing_r, h=3);
    }
    
    translate([0, 0, -7]) cylinder(r = 16/2, h=17);
    
    translate([17/2, 17/2, -7]) cylinder(r=2.5, h=17);
    translate([-17/2, 17/2, -7]) cylinder(r=2.5, h=17);
    translate([-17/2, -17/2, -7]) cylinder(r=2.5, h=17);
    translate([17/2, -17/2, -7]) cylinder(r=2.5, h=17);
    }
...
pdx = 20;
ch = 3;
if(1)translate([0, 0, output_pilot_h+ring_gear_h+4+2.99])
    rotate([0, 0, 180])
difference(){
    union(){
//        translate([0, 0, 1.5]) cube([42+2*2, 42+2*2, 3], center=true);
        translate([-pdx, -pdx, -ph]) cylinder(r=3, h=ph);
        translate([pdx, pdx, -ph]) cylinder(r=3, h=ph);
        translate([-pdx, pdx, -ph]) cylinder(r=3, h=ph);
        translate([pdx, -pdx, -ph]) cylinder(r=3, h=ph);
        translate([0, 0, -ph+ch/2]) cube([45, 45, ch], center=true);
    }
        translate([-pdx, -pdx, -ph-0.5]) cylinder(r=2, h=ph+1);
        translate([pdx, pdx, -ph-0.5]) cylinder(r=2, h=ph+1);
        translate([-pdx, pdx, -ph-0.5]) cylinder(r=2, h=ph+1);
        translate([pdx, -pdx, -ph-0.5]) cylinder(r=2, h=ph+1);
        translate([0, 0, -ph+ch/2+1]) cube([35, 35, ch+3], center=true);
    
    translate([0, 0, -1]) cylinder(r = 11, h=7);
    
    translate([32/2, 32/2, -1]) cylinder(r=2, h=7);
    translate([-32/2, 32/2, -1]) cylinder(r=2, h=7);
    translate([-32/2, -32/2, -1]) cylinder(r=2, h=7);
    translate([32/2, -32/2, -1]) cylinder(r=2, h=7);
}
